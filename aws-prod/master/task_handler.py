        ...
        for message in consumer:
            try:
                if message.value.get('subtask_id') in subtasks:
                    ...
                    process_subtask_result(result,session_id)
...
def process_subtask_result(result,session_id):
    ...
    update_subtask(subtask_id, session_id, job_id, status, result, redis_client)

    # Check if all subtasks for this job are complete
    check_job_completion(job_id,session_id)


def check_job_completion(job_id,session_id):
    ...
    redis_client.set(f"jobs:{job_id}:completion", completion_percentage)

    # If all subtasks are complete, mark job as complete
    if completed_subtasks == total_subtasks:
        logger.info(f"Job {job_id} completed. All {total_subtasks} subtasks finished.")
        ...
        redis_client.set(f"jobs:{job_id}:result", json.dumps(final_result))
        redis_client.set(f"jobs:{job_id}:status", "completed")
        job_status = redis_client.get(f"jobs:{job_id}:status")
        logger.info(job_status)
...
def aggregate_results(redis_client, task):
    """
    ...
    subtask_ids = job_data.get('subtasks', [])

    # Collect results from all completed subtasks
    model_results = []
    for subtask_id in subtask_ids:
        # Skip the aggregation task itself
        if "aggregation" in subtask_id:
            continue

        subtask_key = f"subtask:{subtask_id}"
        subtask_data = json.loads(redis_client.get(subtask_key))

        if subtask_data.get('status') == 'completed' and 'result' in subtask_data:
            model_results.append({
                'subtask_id': subtask_id,
                'parameters': subtask_data.get('parameters', {}),
                'metrics': subtask_data.get('result', {})
            })

    # Find the best model
    if search_type in ["GridSearchCV", "RandomizedSearchCV"]:
        # Sort by mean CV score for both classification and regression
        sorted_results = sorted(model_results,
                                key=lambda x: x['metrics'].get('mean_cv_score', 0),
                                reverse=True)
        ...
        # For non-search tasks, just return the collected results
        return {
