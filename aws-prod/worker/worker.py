...
def update_task_status(task_id, status, result=None):
    ...
    Args:
        ...
        result (dict, optional): Task results to store
    """
    try:
        subtask_key = f"subtask:{task_id}"

        # Get current task data
        task_data = json.loads(redis_client.get(subtask_key))
        ...
        # Save updated task
        redis_client.set(subtask_key, json.dumps(task_data))

        # If task is completed or failed, update the job status
...
def update_job_progress(job_id, is_success):
    """
    ...
    Args:
        ...
        is_success (bool): Whether the subtask was completed successfully
    """
    try:
        ...
        if is_success:
            job_data['completed_subtasks'] += 1

        # Update job status if all subtasks are completed
        if job_data['completed_subtasks'] >= job_data['total_subtasks']:
